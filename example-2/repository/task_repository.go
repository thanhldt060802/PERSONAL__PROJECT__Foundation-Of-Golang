...
func (taskRepository *taskRepository) GetAvailable(ctx context.Context) (int64, error) {
	...
	task := &model.Task{}
	err = tx.NewSelect().Model(task).Where("status = 'PENDING'").Limit(1).For("UPDATE SKIP LOCKED").Scan(ctx)
	...
	if _, err := tx.NewUpdate().Model(task).Where("id = ?", task.Id).Exec(ctx); err != nil {
		return -1, err
	}
