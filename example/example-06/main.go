...
func main() {
	...
	myNode.SpawnRegister(gen.Atom("worker_supervisor"), app.FactoryWorkerSupervisor, gen.ProcessOptions{})

	fmt.Println()
	fmt.Println()
	time.Sleep(2 * time.Second)

	myNode.Send(gen.Atom("worker_1"), types.SimpleMessage{Data: "Data 1"})
	myNode.Send(gen.Atom("worker_2"), types.SimpleMessage{Data: "Data 2"})
	myNode.Send(gen.Atom("worker_3"), types.SimpleMessage{Data: "Data 3"})
	myNode.Send(gen.Atom("worker_4"), types.SimpleMessage{Data: "Data 4"})
	myNode.Send(gen.Atom("worker_5"), types.SimpleMessage{Data: "Data 5"})
